...
def _assert_tensor_shape(tensor, shape, display_name):
    ...
    wrong_dimension = [
        ten_dim for ten_dim, cor_dim in zip(tensor_shape, shape)
...
def test_optimize(optimize):
    ...
    assert test.min() != 0 or test.max(
    ) != 0, 'Training operation not changing weights.'
...
def test_for_kitti_dataset(data_dir):
    ...
    training_labels_count = len(
        glob(
            os.path.join(kitti_dataset_path,
